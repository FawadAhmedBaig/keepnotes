...
import { Search, RefreshCw, Sun, Moon, LogOut, Settings, Lightbulb, Menu } from "lucide-react";
...
export function Header({
  ...
  return (
    <header className="sticky top-0 z-50 flex items-center h-14 sm:h-16 gap-2 sm:gap-4 px-4 border-b border-border bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
      ...
      {/* 1. Branding Section with Lucide Icon */}
      <div className="flex items-center gap-2 mr-1 sm:mr-4 shrink-0">
        ...
        <div className="flex items-center gap-2 ml-1">
          ...
          <span className="font-bold text-lg sm:text-xl hidden lg:block tracking-tight">
      ...
      <div className="flex items-center gap-1">
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="w-64 mt-2 rounded-xl shadow-xl">
            {/* ... keeping the same dropdown items ... */}
            <div className="flex items-center gap-3 p-3">
              ...
              <div className="flex flex-col overflow-hidden">
                <span className="text-sm font-semibold truncate leading-none mb-1">{session?.user?.name}</span>
                <span className="text-xs text-muted-foreground truncate italic">{session?.user?.email}</span>
              </div>
            </div>
            <DropdownMenuSeparator />
